...
namespace HashState
{
    std::uint32_t h0{ sqrtHash[0] };
    std::uint32_t h1{ sqrtHash[1] };
    std::uint32_t h2{ sqrtHash[2] };
    std::uint32_t h3{ sqrtHash[3] };
    std::uint32_t h4{ sqrtHash[4] };
    std::uint32_t h5{ sqrtHash[5] };
    std::uint32_t h6{ sqrtHash[6] };
    std::uint32_t h7{ sqrtHash[7] };
}
...
std::vector<std::array<std::uint32_t, 64>> createMessageSchedule(const Binary& binary)
{
    std::vector<std::array<uint32_t, 64>> result{};

    for (std::size_t chunkStart{0}; chunkStart < binary.size(); chunkStart += 64)
    {
        ...
        result.push_back(messageBlock);
    }

    return result;
}

void compressMessage(const std::array<std::uint32_t, 64>& cbrtConstants, const std::array<std::uint32_t, 64>& messageSchedule)
{
    std::uint32_t a{ HashState::h0 };
    std::uint32_t b{ HashState::h1 };
    std::uint32_t c{ HashState::h2 };
    std::uint32_t d{ HashState::h3 };
    std::uint32_t e{ HashState::h4 };
    std::uint32_t f{ HashState::h5 };
    std::uint32_t g{ HashState::h6 };
    std::uint32_t h{ HashState::h7 };
...
std::array<std::uint32_t, 8> compressMessage(const std::vector<std::array<std::uint32_t, 64>>& messageSchedule)
{
    using namespace HashState;

    for (const std::array<std::uint32_t, 64>&messageChunk : messageSchedule)
    {
        compressMessage(cbrtHash, messageChunk);
    }
...
int main(int argc, [[maybe_unused]] char* argv[])
{
    ...
    std::vector<std::array<std::uint32_t, 64>> messageSchedule{ createMessageSchedule(binary) };

    std::array<std::uint32_t, 8> hash{ compressMessage(messageSchedule) };
